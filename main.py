...
def openApp(command):
    if "calculator" in command:
        ...
        speak("Opening Paint...")
        os.startfile(
            'C:\\Program Files\\WindowsApps\\Microsoft.Paint_11.2504.551.0_x64__8wekyb3d8bbwe\\PaintApp\\mspaint.exe')
        ...
        os.startfile(
            'C:\\Program Files\\WindowsApps\\Microsoft.WindowsCamera_2025.2505.2.0_x64__8wekyb3d8bbwe\\WindowsCamera.exe')
        ...
        os.startfile(
            'C:\\Program Files\\WindowsApps\\5319275A.WhatsAppDesktop_2.2528.4.0_x64__cv1g1gvanyjgm\\WhatsApp.exe')

    elif "chatgpt app" in command:
        speak("Opening ChatGPT App...")
        os.startfile(
            'C:\\Program Files\\WindowsApps\\OpenAI.ChatGPT-Desktop_1.2025.202.0_x64__2p2nqsd0c76g0\\app\\ChatGPT.exe')
...
def closeApp(command):
    ...
    # Only return False if no app was found in the command
    return app_found
...
def process_command(query):
    ...
    # Check for social media commands
    if any(platform in query for platform in ['facebook', 'discord', 'whatsapp web', 'instagram',
                                              'youtube', 'linkedin', 'tiktok', 'twitter',
                                              'snapchat', 'reddit', 'chatgpt web']):
        social_media(query)
        return True
    ...
    elif any(app_cmd in query for app_cmd in ["open calculator", "open notepad", "open paint",
                                              "open word", "open excel", "open browser", "open opera",
                                              "open explorer", "open cmd", "open settings", "open camera",
                                              "open zoom", "open vs code", "open py-charm",
                                              "open whatsapp app", "open chatgpt app", "open chrome",
                                              "open microsoft edge"]):
        openApp(query)
        return True

    # Check for close app commands
    elif any(close_cmd in query for close_cmd in ["close calculator", "close notepad", "close paint",
                                                  "close word", "close excel", "close browser",
                                                  "close microsoft edge", "close opera", "close explorer",
                                                  "close cmd", "close settings", "close video player",
                                                  "close camera", "close zoom", "close vs code",
                                                  "close py-charm", "close whatsapp app", "close chatgpt app",
                                                  "close chrome"]):
        app_found = closeApp(query)
        return app_found

    # Check for exit command
    elif "exit" in query:
        speak("Goodbye Ahmad! Have a great day!")
        sys.exit()
        return True
...
if __name__ == "__main__":
    ...
    # Process the command
    command_processed = process_command(query)

    # If command wasn't processed, give generic error message
    if not command_processed:
        speak("Sorry, I did not understand the command.")
        print("Available commands include:")
        print("- Social Media: 'facebook', 'youtube', 'instagram', etc.")
        print("- Apps: 'open calculator', 'close notepad', etc.")
        print("- Volume: 'volume up', 'volume down', 'mute'")
        print("- Schedule: 'schedule' or 'university time table'")
        print("- Exit: 'exit'")
